...
/** Design doc section 6.7: every status code in the error catalogue, asserted against a mocked service. */
@WebMvcTest(DocumentController.class)
@Import(EtagSupport.class)
class DocumentControllerWebMvcTest {
    ...
    private Document sampleDocument(long version) {
        ...
        return new Document(UUID.fromString("0f8d3b2a-6c4e-4a91-9d7f-2b5c8e1a3d40"), "Notes", "hello",
    ...
    void createReturns201WithLocationAndETag() throws Exception {
        when(documentService.create(any(), any())).thenReturn(sampleDocument(1));
    ...
    void getReturns200WithETagAndNoStore() throws Exception {
        ...
        when(documentService.get(id)).thenReturn(sampleDocument(7));
    ...
    void malformedUuidPathReturns400WithInvalidDocumentIdCode() throws Exception {
        mockMvc.perform(get("/api/v1/documents/{id}", "not-a-uuid"))
    ...
    void updateContentWithoutIfMatchReturns428() throws Exception {
        mockMvc.perform(put("/api/v1/documents/{id}/content", UUID.randomUUID())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"content\":\"hi\"}"))
    ...
    void updateContentSuccessReturns200WithNewETagAndNoContentField() throws Exception {
        UUID id = UUID.randomUUID();
        when(documentService.updateContent(eq(id), eq(7L), anyString(), any())).thenReturn(sampleDocument(8));

        mockMvc.perform(put("/api/v1/documents/{id}/content", id)
                        .header("If-Match", "\"7\"")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void updateContentVersionMismatchReturns412WithCurrentState() throws Exception {
        UUID id = UUID.randomUUID();
        when(documentService.updateContent(eq(id), eq(7L), anyString(), any()))
    ...
    void renameSuccessReturns200() throws Exception {
        UUID id = UUID.randomUUID();
        when(documentService.rename(eq(id), eq(3L), anyString())).thenReturn(sampleDocument(4));

        mockMvc.perform(patch("/api/v1/documents/{id}", id)
                        .header("If-Match", "\"3\"")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void deleteSuccessReturns204() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/documents/{id}", id).header("If-Match", "\"3\""))
